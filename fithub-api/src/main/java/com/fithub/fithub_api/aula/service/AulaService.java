...
public class AulaService implements AulaIService {
    ...
    public Aula buscarPorId(Long id) {
        return aulaRepository.findById(id).orElseThrow(
                () -> new EntityNotFoundException("Aula com ID : " + id + " nao encontrado. "));
    }
    ...
    public Aula editarAula(Long id, AulaUpdateDto updateDto, Usuario usuarioLogado) {
        ...
        // Apenas o dono (instrutor) da aula ou um ADMIN (ROLE_PERSONAL) podem editar
        validarPermissao(aulaExistente, usuarioLogado);
    ...
    /**
     * Novo método para permitir o cancelamento manual da aula.
     */
    @Transactional
    public void cancelarAula(Long id, Usuario usuarioLogado) {
        Aula aula = buscarPorId(id);
        validarPermissao(aula, usuarioLogado);

        aula.setStatus(StatusAula.CANCELADA); // Impede que o Scheduler processe presenças
        aulaRepository.save(aula);
        log.info("Aula ID {} cancelada pelo usuário {}", id, usuarioLogado.getUsername());
    }
    ...
    private void validarPermissao(Aula aula, Usuario usuario) {
        boolean isOwner = aula.getInstrutor().getId().equals(usuario.getId());
        boolean isAdmin = usuario.getPerfil().getNome().equals("ROLE_PERSONAL");

        if (!isOwner && !isAdmin) {
            log.info("Usuario {} nao tem permissao para alterar a aula {}", usuario.getUsername(), aula.getId());
            throw new AccessDeniedException("Você não tem permissão para alterar esta aula.");
        }
    }
}